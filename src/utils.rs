...
impl SieveOfAtkin {
    ...
    fn init(&mut self) {
        ...
        std::thread::spawn(move || {
            SieveOfAtkin::algorithm_3_1(1, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(13, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(17, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(29, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(37, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(41, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(49, &tx1, sieve_len);
            SieveOfAtkin::algorithm_3_1(53, &tx1, sieve_len);
            println!("3.1 OK");
        });
        std::thread::spawn(move || {
            SieveOfAtkin::algorithm_3_2(7, &tx2, sieve_len);
            SieveOfAtkin::algorithm_3_2(19, &tx2, sieve_len);
            SieveOfAtkin::algorithm_3_2(31, &tx2, sieve_len);
            SieveOfAtkin::algorithm_3_2(43, &tx2, sieve_len);
            println!("3.2 OK");
        });
        std::thread::spawn(move || {
            SieveOfAtkin::algorithm_3_3(11, &tx3, sieve_len);
            SieveOfAtkin::algorithm_3_3(23, &tx3, sieve_len);
            SieveOfAtkin::algorithm_3_3(47, &tx3, sieve_len);
            SieveOfAtkin::algorithm_3_3(59, &tx3, sieve_len);
            println!("3.3 OK");
        });
        for (k, delta) in rx {
    ...
    fn algorithm_3_1(delta: i32, tx: &std::sync::mpsc::Sender<(i64, i32)>, sieve_len: i64) {
        for f in 1..=15 {
            for g in (1..=30).step_by(2) {
                let quadratic = 4 * f * f + g * g;
                if delta == quadratic % 60 {
                    SieveOfAtkin::algorithm_4_1(delta, f, g, quadratic / 60, tx, sieve_len);
                }
            }
        }
    }
    fn algorithm_3_2(delta: i32, tx: &std::sync::mpsc::Sender<(i64, i32)>, sieve_len: i64) {
        for f in (1..=10).step_by(2) {
            for g in [2, 4, 8, 10, 14, 16, 20, 22, 26, 28] {
                let quadratic = 3 * f * f + g * g;
                if delta == quadratic % 60 {
                    SieveOfAtkin::algorithm_4_2(delta, f, g, quadratic / 60, tx, sieve_len);
                }
            }
        }
    }
    fn algorithm_3_3(delta: i32, tx: &std::sync::mpsc::Sender<(i64, i32)>, sieve_len: i64) {
        for (f, gstart) in (1..=10).zip([2, 1].into_iter().cycle()) {
            for g in (gstart..=30).step_by(2) {
                ...
                if delta == quadratic.rem_euclid(60) {
                    SieveOfAtkin::algorithm_4_3(
                        delta,
                        f,
                        g,
                        quadratic.div_euclid(60),
                        tx,
                        sieve_len,
                    );
                }
    ...
    fn algorithm_4_1(
        ...
        g: i32,
        h: i32,
        tx: &std::sync::mpsc::Sender<(i64, i32)>,
        sieve_len: i64,
    ) {
