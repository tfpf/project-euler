...
pub fn solve() -> i64 {
    ...
    // of the order of `a.pow(2) * b.pow(2) / 4`, which must be close to
    // 2000000. Hence, we can make an educated guess about the range to test
    // `a` and `b` for.
    let mut result = (0, 0, i64::MAX);
    for (a, sum_till_a) in (1..).zip(utils::Polygonal::new(3)).take(200) {
        for (b, sum_till_b) in (1..).zip(utils::Polygonal::new(3)).take(200) {
